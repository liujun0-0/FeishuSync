# WhatsApp Direct Send API 功能使用与接入调研
---
> 版本：v1.1（调研稿 + 实测补充）
...
**命名模板内容 = 最新一次发送（2026-08-18 实测，§5.7 用例 15）**：同一 `template_name` 发送**不同内容**时，模板 BODY **被最新一次发送内容覆盖（模板 id 不变），即归因（template_id）稳定**、内容**不持久**。实测 `order_shipment_update`（id=`3489704777856131`）三次发不同文案（发货→已送达→订阅提醒），id 恒不变、BODY 依次覆盖，均正常投递、无 132021。**业务影响**：`template_name` 只提供固定归因锚点，不能依赖它持久保存文案。  
### 4.3 自动生成模板的清理策略
自动生成模板分**两阶段清理**：  
<table>
<tr>
...
### 5.7 实测记录（2026-08-17，结论 + 代表用例）
> 完整实测数据（13 组用例的完整 curl / 同步响应 / Meta 回调）见附录文件：`实测记录-附录.md`（测试环境专用）。
> 测试环境：网关端点 `.../sino_channel_facebook/ZRY8eB2neSyLx/v24.0/111681701943055/messages`；phone_number_id=`111681701943055`；WABA=`111918461725990`；sender=`8615736742277`；收件 `8618516505516`（BSUID=`CN.1765719894460979`）。  
**实测结论总表**（完整请求/响应/回调见附录对应用例）：  
<table>
<tr>
<td>#  </td>
<td>用例  </td>
...
<td>文本 utility（中文）  </td>
<td>✅ 送达+已读  </td>
<td>自动匹配模板 `template_id=2157766311750751`  </td>
<td>用例 1  </td>
</tr>
<tr>
<td>2  </td>
<td>文本 + TTL（ttl_seconds=30）  </td>
<td>✅ 送达+已读  </td>
<td>TTL 生效；相同内容复用同一自动模板  </td>
<td>用例 2  </td>
</tr>
<tr>
<td>3  </td>
<td>文本 authentication  </td>
<td>⚠️ 受理→**异步 ****131047**** 失败**  </td>
<td>同步返回 wamid，随后 statuses webhook 报 failed（超 24h 未回复）  </td>
<td>用例 3  </td>
</tr>
<tr>
<td>4  </td>
<td>独立 CTA 网址按钮（中文）  </td>
<td>✅ 送达  </td>
<td>命中独立模板 `template_id=1602428478079454`  </td>
<td>用例 4  </td>
</tr>
<tr>
<td>5  </td>
...
<tr>
<td>11  </td>
<td>文本 + 商家命名模板（template_name）  </td>
<td>⚠️ 受理→回调仅 `sent`  </td>
<td>named 模板命中 `template_id=3489704777856131`；回调外层出现 `conversationExpirationTime`  </td>
<td>用例 11  </td>
</tr>
<tr>
<td>11 复测  </td>
<td>named 模板重发  </td>
<td>✅ read  </td>
<td>**named 可正常投递**（初测仅 sent 为时序）；DS 消息落 **customer-initiated 免费会话**（`free_customer_service`，billable=false）  </td>
<td>用例 11 复测  </td>
</tr>
<tr>
<td>12  </td>
<td>Call on WhatsApp（voice_call）  </td>
<td>❌ 同步 `138000`  </td>
<td>未启用 calling，无回调  </td>
<td>用例 12  </td>
</tr>
<tr>
<td>13  </td>
<td>BSUID 收件人  </td>
<td>⚠️ 受理→**无回调、未送达**  </td>
<td>Meta 受理并自动生成 APPROVED 模板，但投递链中断（待确认）  </td>
<td>用例 13 + 复测  </td>
</tr>
<tr>
<td>14  </td>
<td>named 模板重名（template_name=morning）  </td>
<td>⚠️ 受理→**异步 ****132021**** 失败**  </td>
<td>与 WABA 内非 DS 模板重名；失败回调 contacts 无 `user_id`  </td>
<td>用例 14  </td>
</tr>
<tr>
<td>14 补  </td>
<td>131000 无法用参数构造  </td>
<td>—  </td>
<td>非法/超长 template_name 均**同步报 100**，131000 仅属 Meta 内部失败  </td>
<td>用例 14 补  <br/>> **补充：用例 15（2026-08-18）**：同一 `template_name` 发不同内容 → 模板 BODY 被最新内容覆盖（id 不变），归因稳定、内容不持久。详见 §4.2。完整实测见附录用例 15。  </td>
</tr>
</table>
**回调结构关键差异**（替代逐一比对完整 JSON）：  
<table>
<tr>
<td>场景  </td>
<td>回调特点  </td>
...
<tr>
<td>成功（sent/delivered/read）  </td>
<td>`statuses[]` 含 `template_id`（DS 新增字段）、`conversation{id,origin,expiration_timestamp}`、`pricing{billable,pricing_model,category,type}`；`contacts[0]` 含 `wa_id` + `user_id`（BSUID）  </td>
</tr>
<tr>
<td>失败（failed）  </td>
<td>网关外层多 `errorCode/errorDesc`；`statuses[].errors[]`（如 131047/132021，含 `code/title/message/error_data.details/href`）；`contacts[0]`**可能不含 ****user_id**（132021 实测）  </td>
</tr>
<tr>
<td>named 模板  </td>
<td>回调外层出现 `conversationExpirationTime`（epoch ms）  </td>
</tr>
<tr>
<td>状态映射  </td>
<td>网关 `status`：1=sent 2=delivered 3=read 4=failed  </td>
</tr>
</table>
**代表用例（完整示例，其余见附录）**：  
**① 文本 utility 成功路径（回调解析基准）**  
```
...
--data '{
    ...
    "to": "8618516505516",
    "type": "text",
    "text": { "body": "你好！这是一条测试消息1004" },
    "category": "utility"
}'
```
同步响应：`{"messaging_product":"whatsapp","contacts":[{"input":"8618516505516","wa_id":"8618516505516"}],"messages":[{"id":"wamid.HBgNODYxODUxNjUwNTUxNhUCABEYEjk2NDlDRjM5NENCRkIxMjBBMAA="}]}`
Meta 回调（`messageData` 内）：`statuses[0]` → `status=read`、`template_id=2157766311750751`、`conversation.origin.type=utility`、`pricing.billable=true`、`pricing.type=regular`；外层 `conversationPricingType=regular`。  
**② authentication → 异步 ****131047****（官方未给，实测关键）**
同步返回 wamid 后，`statuses[0].status=failed`，`errors[0]`：  
```
...
```
→ **131047 是异步错误**（§6 口径以此为准）；authentication DS 消息同样受 24h re-engagement 约束。  
**③ named 模板重名 → 异步 ****132021****（官方未给，实测关键）**
`direct_send_config.template_name=morning`（WABA 内已有非 DS 模板重名）→ 同步受理 wamid，随后 `statuses[0].status=failed`，`errors[0]`：  
```
{"code":132021,"title":"A template with the same name already exists. Choose a different template name","message":"A template with the same name already exists. Choose a different template name","error_data":{"details":"A template with the same name already exists. Choose a different template name"}}
```
无 `href`；失败回调 `contacts[0]` 仅 `wa_id`、**无 ****user_id**。处理建议：换用 WABA 内唯一的 `template_name`。  
**④ DS 模板用于标准模板消息 → 同步 ****132018****（官方错误码表未收录）**
用 `type=template` 按 name+language 引用 DS 生成的模板（自动生成 `auto_generated_*` 或商家命名 `template_name`）→ **同步**报错：  
```
{"error":{"message":"(#132018) There’s an issue with the parameters in your template","code":132018,"type":"OAuthException","error_data":{"messaging_product":"whatsapp","details":"Direct send template can't be used for template message"},"fbtrace_id":"..."}}
```
→ 与 §4.2「双向隔离」一致：DS 模板不能发普通模板消息。  
**⑤ BSUID 收件人（用例 13）— 受理但不可投递**
`recipient=CN.1765719894460979` → 同步受理（wamid 前缀 `HBgTQ04u...` 为 BSUID base64），自动生成 APPROVED 模板，但**无任何回调、手机未收到**——投递链中断，接入前必须专项确认 BSUID 可用性（§2.1）。  
## 6. 模板异常处理
...
- `correct_category` 只能取 **MARKETING** 或 **AUTHENTICATION**（官方原文："The `correct_category` can be `MARKETING` or `AUTHENTICATION`."）
- **触发由 Meta 主动监控**（"Meta actively monitors how messages are categorized"），无手动触发 API；首次发送仍可送达，标记是事后异步完成
- → 告警运营；可对标记模板申请复审（见 §8）；误判→复审，确为营销→改用 Marketing 通道
- 来源：[https://developers.facebook.com/documentation/business-messaging/whatsapp/direct-send/integrity-and-content-guidelines/](https://developers.facebook.com/documentation/business-messaging/whatsapp/direct-send/integrity-and-content-guidelines/)
...
升级路径（官方"Account restrictions roll out incrementally"，**警告 + 限流先全量；7 天 / 30 天 / 永久撤销分阶段启用**，各账户可能未激活）：  
<table>
<tr>
<td>阶段  </td>
...
<tr>
<td>1 警告  </td>
<td>`DIRECT_SEND_UTILITY_CATEGORY_ABUSE_WARN` 或 `_UTILITY_TEMPLATE_ABUSE`  </td>
<td>—  </td>
<td>—  </td>
</tr>
<tr>
<td>2 限流  </td>
<td>`DIRECT_SEND_UTILITY_CATEGORY_ABUSE_RATE_LIMIT`  </td>
<td>含 `restriction_info.expiration`  </td>
<td>`131064`（达上限后拒发）  </td>
</tr>
<tr>
<td>3 二级  </td>
...
## 8. 合规与风控（重点注意事项）
> **官方原文机制**（来源：`integrity-and-content-guidelines`）：_"Meta actively monitors how messages are categorized"_——Meta **主动监控**所有 DS utility 消息，识别为 marketing/authentication 即触发后续；首次发送仍可送达，标记是**事后异步完成。没有手动触发 API**。  
- **模板暂停**：命中低质量暂停模板 → 错误 `132015`。通知：邮件 + `message_template_status_update`（PAUSED）。DS 模板**不能人工重启/编辑/删除**，只能改发送内容重新生成。
- **类别滥用监控**：Meta 监控 utility 中混入营销内容。通知：邮件 + `template_correct_category_detection`（`correct_category=MARKETING` 或 `AUTHENTICATION`，二选一）。可在 **Business Support Home → WhatsApp 账户 → Direct Send 模板更新** 申请复审：**60 天内、每模板仅一次、复审期间不中断发送**。复审邮箱：`wadirectsendapisupport@meta.com`。
- **账户限制升级**：WARN → 流量限制（错 131064）→ 消息限制（7 天）→ 延长（30 天）→ **永久撤销**。通知：邮件 + `account_update`（`restriction_info`）。**官方原文**："Account restrictions roll out incrementally"——警告与限流先全量；7 天 / 30 天 / 永久撤销**分阶段启用**，各 WABA 可能未激活。详细 `violation_type` 与同步错误见 §6.2.3。
- 促销/营销内容请走 **Marketing Messages** 或旧模板路径，勿用 DS。
- **Webhook 订阅要求**（缺哪个就收不到对应事件，必须全订阅）：
  - `template_correct_category_detection` → 入库 `message_template_id`，按模板标记/暂停发送
  - `message_template_status_update` → PAUSED 立即停发该模板
  - `account_update` → `ACCOUNT_RESTRICTION` 监控账户状态，写入 `restriction_info.expiration`
- **查询已标记违规模板（不依赖 webhook，直接 API 拉取）**：
```
GET /{WABA_ID}/message_templates?source=AUTO_GENERATED&correct_category=MARKETING
GET /{WABA_ID}/message_templates?source=AUTO_GENERATED&correct_category=AUTHENTICATION
```
- **运营兜底**：除 webhook 外，Meta 还会**邮件通知 WABA 管理员**——即使 webhook 未配/未订阅，也请保持 WABA 管理员邮箱可接收。
...
<td>**13 组完整 API/响应/回调实测 + 复测**（完整数据见附录 `实测记录-附录.md`）：手机号路径全部类型可送达；named 模板复测 read、落 customer-initiated 免费会话；**131047 异步**；合规内容图片头过检；**BSUID 被 Meta 受理（生成 APPROVED 模板）但不可投递（无回调，待确认）；DS 模板不可用于标准模板消息已实测（132018，2026-08-18）**  </td>
</tr>
<tr>
<td>§5.3  </td>
